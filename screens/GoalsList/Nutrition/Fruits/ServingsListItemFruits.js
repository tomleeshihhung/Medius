...
import { fruitsServingsChanged, fruitsChanged } from './goalsActionsFruits';
...
class ServingsListItemFruits extends Component {
  state = { selectedList: [] }
  ...
  daysSelectedDays = async () => {
    const { fruitsServings, data } = this.props;
  //  this.props.fruitsChanged({ prop: 'fruitsServingsSelected', value: [] });

    if (fruitsServings[data.key].selected === 'false') {
      const array = [0, 1, 2, 3, 4, 5, 6, 7];
      await array.map((obj, i) => {
        return this.props.fruitsServingsChanged({ key: i, value: 'false' });
      });
      this.props.fruitsServingsChanged({ key: data.key, value: 'true' });
      this.props.fruitsChanged({ prop: 'fruitsServingsSelected', value: data.title });
      const newStatus = this.calculateStatus(this.props.fruitsDaysSelected, data.title);
